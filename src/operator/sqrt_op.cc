...
std::shared_ptr<OpBase> BuildSqrtOp(const model::NodeDesc& node, OperatorRegistry::TensorMap& tensors) {
    ...
    auto kernel = CreateHostKernelForTensor("Sqrt", {param.input, op->outputs().front()}, DataType::FP32);
    if (kernel == nullptr) {
        return nullptr;
    }
