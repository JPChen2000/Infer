...
std::shared_ptr<OpBase> BuildExpOp(const model::NodeDesc& node, OperatorRegistry::TensorMap& tensors, const OperatorRegistry::BuildContext& context) {
    ...
    auto kernel = CreateKernelForTensor(context.device, "Exp", {param.input, op->outputs().front()}, DataType::FP32);
