...
std::shared_ptr<OpBase> BuildSubOp(const model::NodeDesc& node, OperatorRegistry::TensorMap& tensors) {
    ...
    auto kernel = CreateHostKernelForTensor("Sub", {param.lhs, param.rhs, op->outputs().front()});
    if (kernel == nullptr) {
        return nullptr;
    }
