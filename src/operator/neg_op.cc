...
std::shared_ptr<OpBase> BuildNegOp(const model::NodeDesc& node, OperatorRegistry::TensorMap& tensors) {
    ...
    auto kernel = CreateHostKernelForTensor("Neg", {param.input, op->outputs().front()}, DataType::FP32);
